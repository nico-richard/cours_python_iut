"""Validation locale : python validation_seance4.py s4_ex1.py tracer_evolution.

Sans argument, valide les dix fonctions d'un fichier seance4.py voisin.
...
from data.validation.seance4 import EXERCICES_SEANCE4


def verifier_fichier(chemin, nom=None):
    exercices = [e for e in EXERCICES_SEANCE4 if nom is None or e.function_name == nom]
    if not exercices:
        print(f"Fonction inconnue : {nom}")
        return 1
    spec = importlib.util.spec_from_file_location("seance4_etudiant", chemin)
...
def main():
    ...
    if arguments and arguments[0] == "--executer":
        try:
            ...
            return 1
    chemin = Path(arguments[0]) if arguments else Path(__file__).with_name("seance4.py")
    ...
    try:
        ...
        print("Délai dépassé (10 s) : vérifier que le programme peut se terminer.")
        return 1
